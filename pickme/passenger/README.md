...
*   **🏗️ Architecture Support:** `armeabi-v7a` (Configured via split APKs)
...
*   **⚙️ Services:** `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MEDIA_PLAYBACK` (Background persistence).
...
*   **🗺️ Integration:** Google Maps SDK for Android.
...
*   **🛡️ Gateway:** **CyberSource** (`api.cybersource.com`).
...
*   **👁️ Feature Extraction:** Utilizes Google ML Kit (`oned_feature_extractor_mobile.tflite`) for high-speed barcode/QR scanning.
...
> [!IMPORTANT]
> **CyberSource Payment Fabric:** Deep integration with CyberSource for 3D Secure, tokenized credit/debit card processing, ensuring industrial-grade financial security.
